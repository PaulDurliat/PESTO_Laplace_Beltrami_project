...
class SphericalHarmonicsDecomposer(object):
  ...
  def fourrier_spherical_harmonics_decomposition(self, Lmax, big_RY=None, verbose=False):
    ...
    minCoord = min(min(self.bx),min(self.by),min(self.bz))
    maxCoord = max(max(self.bx),max(self.by),max(self.bz))
    
    mustCompute = False
    if big_RY is not None:
        M = big_RY.shape[0]
        cM = int(M/2)
        mustCompute = cM+2*maxCoord>=M or cM+2*minCoord<0
        if mustCompute:    
            if verbose:
                print("WARNING: big_RY is too small, Y will be computed.")
        elif big_RY.shape[-1] != (Lmax+1)**2:
            mustCompute = True
            if verbose:
                print("WARNING: wrong Lmax for big_RY, Y will be computed.")
        
    if big_RY is None or mustCompute:
        ...
        for l in range(Lmax+1):
          ...
          for m in range(-l,l+1):
            ...
            Y[:,j] = (-1)**m*myPml.T * np.exp((1.j) * m * self.phi) * np.sqrt(np.complex((2 * l + 1) / (4 * pi) * nm))

            #Check if everything is good
            self.check_NaN(Y,'Y')

    else:
        Y = big_RY[(cM+2*self.by).astype(int),(cM+2*self.bx).astype(int),(cM+2*self.bz).astype(int)]
  ...
  def fourrier_spherical_harmonics_modelisation(self, Lmax):
    ...
    for l in range(Lmax+1):
      ...
      for m in range(-l,l+1):
        ...
        RY[:,j]=(-1)**m*myPml.T * np.exp((1.j) * m * rphi) * np.sqrt(np.complex((2 * l + 1) / (4 * pi) * nm))
